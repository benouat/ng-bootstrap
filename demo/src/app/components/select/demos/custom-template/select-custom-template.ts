...
@Component({
  selector: 'ngbd-select-custom-template',
  templateUrl: './select-custom-template.html',
  styles: [`
      .form-control {
        width: 300px;
      }
...
export class NgbdSelectCustomTemplate {
  states = statesWithFlags.map(state => ({...state, value: state.name.toLowerCase().replace(/ /g, '-')}));
